 ...
 *
 *   1. The TextOS checkout at `--textos-path` must sit at EXACTLY the writer SHA pinned in
 *      `textos/tool.json`, with a clean working tree (no tracked change, no untracked file).
 ...
 *
 * Steps: `site-intelligence`, `prepare --article <id>`, `write --article <id>`.
 */
...
const STEPS = ["site-intelligence", "prepare", "write"] as const;
...
function main(): void {
  ...
  if (!textosPath || !existsSync(join(textosPath, ".git"))) {
    fail("--textos-path <path to a checkout of gangster-nerd/textos-v0> is required.");
  }
  ...
  if (head !== tool.writer.sha) {
    fail(
      `TextOS checkout is at ${head}, but textos/tool.json pins ${tool.writer.sha}. ` +
        `Check out the pinned SHA in that checkout yourself (\`git -C ${textosPath} checkout ${tool.writer.sha}\`); ` +
